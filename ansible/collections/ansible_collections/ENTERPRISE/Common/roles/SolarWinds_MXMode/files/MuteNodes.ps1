...
function Set-NodeSuppression {
    ...
    try {
        ...
        if ($Unmute) {
            ...
            Invoke-SwisVerb $SwisConnection Orion.AlertSuppression ResumeAlerts @(, $Node.Uri, $now) | Out-Null
            Write-Output "Device Unmuted"
        }
        elseif ($Mute) {
            Write-Verbose "Muting $($Node.Uri)"
            Invoke-SwisVerb $SwisConnection Orion.AlertSuppression SuppressAlerts @(, $Node.Uri, $now, $later) | Out-Null
            Write-Output "Device Muted from $($now) to $($later) UTC ($($hours) hours)"
        }
        else {
 ...
 [CmdletBinding()]
    ...
    try {
        ...
        elseif ($Unmanage) {
            ...
            Invoke-SwisVerb $SwisConnection Orion.Nodes Unmanage @("N:$($Node.NodeID)", $now, $later, "false") | Out-Null
            Write-Output "Device Unmanaged from $($now) to $($later) UTC ($($Hours) hours)"
        }
        else {
            throw "Unable to mute or unmute device."
        }
